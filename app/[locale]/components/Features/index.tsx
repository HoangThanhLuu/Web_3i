...
import { GetListsService, GetMenuData } from '../../apis/GetDataHome';
...
const Features = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const dataa = await GetListsService()
      setLists(dataa.data)
  ...
  return (
    <>
      <section
        id="features"
        className="bg-[green]/[.03] py-3"
      >

        <div className="container max-w-full lg:my-14 sm:max-w-screen-xl text-[#002968]">
          <SectionTitle
            title={data?.title}
            paragraph=""
            center
